...
(function (RT) {
  ...
  cloud.signUp = function (email, password, name) {
    return Promise.resolve(cloud.auth.signUp.email({ email: email, password: password, name: name || email.split('@')[0] }))
      .then(unwrap).then(cloud.refreshUser);
  };

  cloud.signIn = function (email, password) {
    return Promise.resolve(cloud.auth.signIn.email({ email: email, password: password }))
      .then(unwrap).then(cloud.refreshUser);
  };
