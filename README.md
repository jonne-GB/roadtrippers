...
> **Volgorde is hier niet vrijblijvend.** Zet eerst Auth en de Data API aan, en voer `sql/schema.sql` pas daarna uit. Het script gebruikt `auth.user_id()` en de rol `authenticated`, en die bestaan pas als die twee aanstaan. Draai je het script te vroeg, dan worden de tabellen wél aangemaakt maar de policies en grants niet — en dan strandt de app op `permission denied for table trips`. Het script controleert dit tegenwoordig zelf en stopt met een duidelijke melding.

### 2.1 Project aanmaken

Maak een project aan (of gebruik een bestaand).
...
4. Ga naar **Auth → Configuration → Domains** en voeg het adres toe waar de app draait, bijvoorbeeld `https://roadtrippers.pages.dev`. **Mét protocol, zonder slash aan het eind.**

Die laatste stap is geen detail: zonder dat weigert Neon elke inlogpoging met *"Invalid origin"*. Alle `localhost`-poorten staan al standaard toe, dus alleen je echte adres moet erbij. Voor preview-deploys kun je een wildcard gebruiken, zoals `https://*.roadtrippers.pages.dev`.
...
### 2.4 Database klaarzetten

1. Open **SQL Editor**. Let op de branch-selector bovenin: kies dezelfde branch als waar de Data API op staat.
2. Plak de complete inhoud van **`sql/schema.sql`** en voer het uit.

Dat maakt drie tabellen (`trips`, `trip_members`, `trip_items`), zet **Row Level Security** aan, maakt de functies `create_trip()`, `join_trip()` en `leave_trip()`, en geeft de rol `authenticated` de rechten die de Data API nodig heeft. RLS is niet optioneel: het is precies wat het veilig maakt dat je browser rechtstreeks met de database praat. Iedereen ziet alleen de reizen waar hij lid van is.

Onderaan het script staat een controle-query. Je hoort drie regels te zien met `rowsecurity = true`. Krijg je in plaats daarvan een melding over `auth.user_id()` of de rol `authenticated`, dan staat stap 2.2 of 2.3 nog niet aan.

### 2.5 `config.js` invullen
...
**Vercel** — kan wel, maar heeft als enige van deze lijst een terminal nodig: Vercel heeft geen upload-via-de-browser zoals Cloudflare en Netlify. Je hebt Node nodig, eenmalig:

```powershell
winget install OpenJS.NodeJS.LTS
# nieuwe terminal openen, anders kent hij npm nog niet
npm i -g vercel
cd C:\Users\jonne\claude_projects\roadtrippers
vercel login
vercel --prod
```

`vercel login` en de eerste `vercel --prod` stellen een paar vragen (project aanmaken, root directory). Antwoord bij "In which directory is your code located?" met `./`. Er is geen build-stap, dus build command en output directory laat je leeg.

In de root staat `vercel.json`. Die zet op alles `Cache-Control: public, max-age=0, must-revalidate`. Dat is bewust: geen enkele bestandsnaam in dit project bevat een hash, dus met agressieve caching blijft een browser op een oude `cloud.js` of `config.js` hangen — precies het soort spookfout dat je uren kost. `must-revalidate` laat de browser nog steeds een ETag sturen, dus ongewijzigde bestanden kosten maar een 304.

> **Na élke nieuwe deploy-URL: voeg het adres toe in Neon.** Auth → Configuration → Domains, mét protocol en zonder slash. Doe je dat niet, dan krijg je bij het inloggen *"Invalid origin"*. Vercel geeft bovendien iedere preview-deploy een eigen subdomein, dus zet daar een wildcard neer (`https://*.vercel.app` is te ruim — gebruik `https://*-jouwnaam.vercel.app`) of werk alleen met je productie-URL.
...
**"Invalid origin" bij inloggen of registreren** — het adres waar de app draait staat niet in Neons lijst met vertrouwde domeinen. Kijk in je adresbalk, neem het stuk tot aan de eerste `/` na de domeinnaam (bijvoorbeeld `https://roadtrippers.pages.dev`) en voeg dat toe onder **Auth → Configuration → Domains**. Protocol erbij, geen slash erachter. Localhost hoeft niet: alle localhost-poorten zijn al toegestaan.
